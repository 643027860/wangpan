<template>
  <div class="text">
    ...
    <div style="border-bottom: 1px solid rgb(243,243,243)"><p style="margin-left: 15px;margin-bottom: 10px">全部文档:</p></div>
    ...
    <div v-loading="textLoding" class="main_list" style="height: 700px">
      <div v-if="displayTextImg" style="width: 350px;height:400px;background-color: rgba(0,0,0,0.6);position: absolute;z-index: 9999;left: 40%;top: 30%">
        <div class="block">
          <el-image
            class="img"
            :src="'http://47.112.132.177:8092/qrcode/createCommonQRCode?id='+TextId"
      ...
      <div v-if="!textLoding">
        <div v-if="texts.length>0">
          <div v-for="(text,index) in texts" :key="index">
            <li :id="'text'+text.id" class="text_lists" @mouseover="()=>showP(text.id)" @mouseout="()=>hideP(text.id)">
              <i class="el-icon-document" style="float: left;font-size: 26px!important;color: rgb(77,151,255);"/>
              ...
              <span :id="'tp'+text.id" style="display: inline-block;position: absolute;left: 50%;margin: 0;" v-show="false">
                <i class="el-icon-download list_hover down_list" style="font-size: 20px!important;cursor: pointer;margin: 0px 10px;color: rgb(77,151,255);" @click="()=>downloadText(text.id)"/>
                <i class="el-icon-delete list_hover" style="font-size: 20px!important;cursor: pointer;margin: 0px 10px;color: rgb(77,151,255);" @click="()=>deleteText(text.id)"/>
              </span>
            </li>
          </div>
        </div>
        <div v-else>
          <p style="margin:0;margin-top: 20%;margin-left: 38%;display: inline-block;font-size: 28px;">还未上传任何文本</p>
        </div>
      </div>
    </div>
