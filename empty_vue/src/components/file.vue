<template>
  <div class="file">
    ...
    <div style="border-bottom: 1px solid rgb(243,243,243)"><p style="margin-left: 15px;margin-bottom: 10px">全部压缩包:</p></div>
      ...
      <div v-if="!fileLoding">
        <div v-if="files.length>0">
          <div v-for="(file,index) in files" :key="index">
            <li :id="'file'+file.id" class="file_lists" @mouseover="()=>showP(file.id)" @mouseout="()=>hideP(file.id)">
              ...
              <span :id="'fp'+file.id" style="display: inline-block;position: absolute;left: 50%;margin: 0;" v-show="false">
        ...
        <div v-else>
          <p style="margin:0;margin-top: 20%;margin-left: 38%;display: inline-block;font-size: 28px;">还未上传任何其他压缩包</p>
        </div>
      </div>
    </div>
...
<style scoped>
  .file_lists span,.list_type_name,.erweima{
  ...
  .file_lists:hover,.list_type_name:hover{
  ...
  .file_lists .list_name{
    font-size: 14px;
    margin-left: 10px;
    position: relative;
    cursor: pointer;
  }
  .file_lists .list_name:hover{
